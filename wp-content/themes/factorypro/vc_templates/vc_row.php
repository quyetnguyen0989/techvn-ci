 ...
 */
$el_class = $full_height = $full_width = $fullwidth = $equal_height = $flex_row = $columns_placement = $content_placement = $parallax = $parallax_image = $css = $el_id = $video_bg = $video_bg_url = $background_position = $video_bg_parallax = '';
$output = $after_output = '';
...
$output .= '<section ' . implode( ' ', $wrapper_attributes ) . ' style="'.$background_position.'">';
if ( $fullwidth != 'yes' ) {
    $output .= '<div class="row">'.wpb_js_remove_wpautop( $content ).'</div>';
}else{
    $output .= '<div class="row full-width">'.wpb_js_remove_wpautop( $content ).'</div>';
}
$output .= '</section>'; 
$output .= $after_output;
